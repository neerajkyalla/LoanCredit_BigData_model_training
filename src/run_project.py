...
def ingest_validate_with_lineage(spark, run_id: str):
    ...
    quality = clean.agg(
        F.avg(F.when(F.col("invalid_record"), 1.0).otherwise(0.0)).alias("invalid_rate"),
        F.avg("row_missing_fields").alias("avg_missing_fields_per_row"),
        F.avg(F.when(F.col("target") == 1, 1.0).otherwise(0.0)).alias("approval_rate"),
        F.avg(F.when(F.col("target") == 0, 1.0).otherwise(0.0)).alias("rejection_rate"),
    )
...
def run_scalability_experiments(spark, base_df):
    ...
    for partitions in [2, 4, 8, 16]:
        ...
        rows.append(("strong", partitions, elapsed))
    ...
    for partitions, mult in weak_settings:
        ...
        elapsed = time.time() - t0
        rows.append(("weak", partitions, elapsed))

    scale_pdf = pd.DataFrame(rows, columns=["scaling_type", "resource_level", "elapsed_seconds"])

    scale_pdf["performance_index"] = np.nan
    for scaling_type in ["strong", "weak"]:
        subset = scale_pdf[scale_pdf.scaling_type == scaling_type]
        base_time = subset[subset.resource_level == subset.resource_level.min()]["elapsed_seconds"].iloc[0]
        scale_pdf.loc[scale_pdf.scaling_type == scaling_type, "performance_index"] = base_time / subset["elapsed_seconds"].values
...
def write_summary_json(run_id: str, spark_metrics_df, sklearn_metrics_pdf, ci_tuple, business, scaling_pdf):
    summary = {
        ...
        "scaling_summary": {
            "strong_scaling_points": int((scaling_pdf.scaling_type == "strong").sum()),
            "weak_scaling_points": int((scaling_pdf.scaling_type == "weak").sum()),
            "lowest_estimated_cost_usd": float(scaling_pdf["estimated_cost_usd"].min()),
        },
        "notes": [
